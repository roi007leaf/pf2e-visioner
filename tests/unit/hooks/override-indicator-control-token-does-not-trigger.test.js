...
describe('Override indicator should not trigger on controlToken', () => {
  beforeEach(() => {
    ...
    delete global.__pf2eVisionerMultiLevelControlView;
  ...
  test('onCanvasReady does not duplicate singleton hook registrations', async () => {
    const { onCanvasReady } = await import('../../../scripts/hooks/lifecycle.js');
    await onCanvasReady();
    const firstControlHookCount = global.Hooks.on.mock.calls.filter(
      (c) => c?.[0] === 'controlToken',
    ).length;

    await onCanvasReady();
    const secondControlHookCount = global.Hooks.on.mock.calls.filter(
      (c) => c?.[0] === 'controlToken',
    ).length;
  ...
  test('controlToken schedules AVS recalculation for the controlled token', async () => {
    ...
    const trackerCb = controlTokenCbs.find((cb) => String(cb).includes('trackControlTokenSession'));
    const recalcCb = controlTokenCbs.find(
      (cb) =>
    ...
    expect(
      global.window.pf2eVisioner.services.autoVisibilitySystem.recalculateForTokens,
  ...
  test('deselecting the final observer refreshes multi-level surface occlusion', async () => {
    const updateSystemHiddenTokenHighlights = jest.fn().mockResolvedValue(undefined);
    jest.doMock('../../../scripts/services/visual-effects.js', () => ({
      updateWallVisuals: jest.fn(),
      updateWallIndicatorsOnly: jest.fn(),
  ...
  test('deselecting the final observer restores the multi-level view from before selection', async () => {
    const updateSystemHiddenTokenHighlights = jest.fn().mockResolvedValue(undefined);
    jest.doMock('../../../scripts/services/visual-effects.js', () => ({
      updateWallVisuals: jest.fn(),
      updateWallIndicatorsOnly: jest.fn(),
    ...
    const view = jest.fn().mockImplementation(async ({ level }) => {
      global.canvas.level = { id: level };
    });
    global.canvas.perception = { update: jest.fn() };
    global.canvas.level = { id: 'upper-deck' };
    global.canvas.scene = {
      id: 'scene-1',
      levels: { size: 3 },
      getSurfaces: jest.fn(),
    ...
    const { captureMultiLevelViewBeforeControl } = await import(
      '../../../scripts/services/Detection/multi-level-control-view.js'
    );
    const { onCanvasReady } = await import('../../../scripts/hooks/lifecycle.js');
    await onCanvasReady();
    ...
    captureMultiLevelViewBeforeControl(token);
    global.canvas.tokens.controlled = [token];
    trackerCb(token, true);

    // Core switches to the controlled token's level after firing controlToken.
    global.canvas.level = { id: 'main-deck' };
    global.canvas.tokens.controlled = [];
    trackerCb(token, false);
    await restoreIndicatorsCb(token, false);
    jest.advanceTimersByTime(1);
    ...
    expect(view).toHaveBeenCalledWith({ level: 'upper-deck' });
  });
