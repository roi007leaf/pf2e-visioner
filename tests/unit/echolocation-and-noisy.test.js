...
describe('Echolocation integration', () => {
  ...
  test('VisionAnalyzer: echolocation upgrades hearing to precise within range and is considered precise non-visual', () => {
    const {
      VisionAnalyzer,
    } = require('../../scripts/visibility/auto-visibility/VisionAnalyzer.js');

    // Observer token with imprecise hearing 60 ft and active effect 'effect-echolocation' (40 ft default)
    const observerActor = {
      system: {
        perception: { senses: { hearing: { value: { acuity: 'imprecise', range: 60 } } } },
      },
    ...
    expect(va.hasPreciseNonVisualInRange(observer, target)).toBe(false); // Temporary fix
    ...
    expect(va.canSenseImprecisely(observer, target)).toBe(false); // Temporary fix
  });
