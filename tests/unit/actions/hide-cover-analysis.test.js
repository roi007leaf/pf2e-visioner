...
describe('hide cover analysis', () => {
  ...
  test('uses manual cover before automatic cover detection', async () => {
    ...
    const { buildHideAutoCoverData } = await import(
    ...
    const result = await buildHideAutoCoverData({
      actionData: { actor: createMockToken({ id: 'hider' }) },
  ...
  test('turns roll-specific cover override into display metadata', async () => {
    ...
    const result = await buildHideAutoCoverData({
      actionData: {
        actor: createMockToken({ id: 'hider' }),
