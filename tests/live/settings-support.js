...
function authorize(worldId, key) {
  ...
  if (!ALLOWED.has(key) || game.settings.settings.get(`${MODULE}.${key}`)?.scope !== 'world') throw Error('Setting is not allowed for live QA');
}
function stored(key) { return game.settings.storage.get('world').find(entry => entry.key === `${MODULE}.${key}`); }
export function captureSettings({ worldId, keys }) {
  return { worldId, settings: keys.map(key => {
    authorize(worldId, key);
    return { key, value: game.settings.get(MODULE, key), existed: !!stored(key) };
  }) };
}
...
export async function restoreSettings(record) {
  ...
  for (const { key, value, existed } of record.settings) {
    ...
    if (game.settings.get(MODULE, key) !== value || !!stored(key) !== existed) throw Error(`Setting restoration failed: ${key}`);
  }
}
