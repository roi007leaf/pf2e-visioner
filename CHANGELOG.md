...
- **Live scent soundwaves survive movement**: Targets hidden by imprecise senses now keep their soundwave indicator driven by live range, line of sight, and lighting through multi-state movement paths (undetected to hidden to observed), instead of skipping the soundwave stage or flashing at the end of a move.
- **AVS overrides win during movement**: An active hidden AVS override now keeps the soundwave presentation through moves regardless of the live vision polygon, and moving into a position that disagrees with the override shows the AVS change indicator.
- **Movement no longer teleports tokens**: AVS flag persistence now waits for Foundry's movement animation, fixing tokens snapping to their destination mid-walk; rapid back-and-forth moves and tangent paths no longer flicker the soundwave or latch tokens hidden.
- **Orphaned soundwave visuals are reclaimed**: Soundwave rings left visible (or stuck hidden) after movement, drag release, or AVS batches are now swept — hiding stale rings, restoring missing rings and detection filters for still-qualifying pairs, and re-testing visibility for targets that stopped qualifying so they no longer linger until hovered.
- **Hidden-via-hearing targets keep soundwaves through darkness**: Targets the AVS marks hidden by hearing (for example on the far side of a magical darkness source) now keep their soundwave during observer movement instead of disappearing until the move settles.
- **Deafened observers still perceive sense-hidden tokens**: Observers with no non-visual detection mode (for example deafened, scent-only creatures) now render tokens hidden by their imprecise senses with the soundwave indicator, instead of not rendering them at all.
- **Leaving imprecise-sense range hides targets promptly**: Moving out of scent range now hides the target about half a second after the AVS commits undetected, instead of leaving a stale rendered token until hover; the anti-flicker range buffer now only applies while a movement is in flight.
- **Range checks match system distance**: The live soundwave gate now measures range with the system's token distance (bounds-based) instead of center math, fixing boundary squares where a token rendered nothing but its condition icons because the renderer and the AVS disagreed about range.
- **Tremorsense respects elevation**: Tremorsense now only detects when the observer and target share the same elevation and that elevation is the ground, instead of ignoring manual elevation changes.
- **Invisible creatures behind blocking walls become undetected**: The "went invisible while observed" remembered-position rule now requires line of sight to still exist, so walking behind a sight- and sound-blocking wall makes the invisible creature undetected instead of pinning it hidden with floating condition icons.
...
- **Pending movement detection keeps libWrapper chained**: Canvas visibility suppression now still calls Foundry's wrapped `testVisibility` function before forcing a Visioner-hidden result, preventing libWrapper from unregistering the wrapper during movement visibility refreshes.
...
- **Deselecting an observer restores GM visibility**: Releasing the last controlled observer now clears observer-specific hidden-token overlays and render locks without falling back to the stale selection, then refreshes no-observer vision so hidden targets become visible again.
...
- **Rules-accurate Avoid Notice visibility states**: Stealth initiative is now compared against each enemy observer's Perception DC. If the Stealth initiative meets or exceeds the DC, the stealther becomes `unnoticed`; otherwise Visioner leaves the pair to regular AVS visibility calculations.
...
- **Initial stealth override lifecycle**: Encounter-start `unnoticed` overrides preserve any previous AVS override for the same observer-target pair and restore it when the encounter-start override is removed.
...
- **Only enemy observers participate in encounter stealth setup**: Allies are no longer considered when deciding whether a stealth initiative combatant is initially unnoticed.
