...
export class TokenEventHandler {
    constructor(systemStateProvider, visibilityStateManager, spatialAnalyzer, exclusionManager, overrideValidationManager, positionManager, cacheManager = null) {
    ...
    handleTokenUpdate(tokenDoc, changes) {
        ...
        if (emitterMoved) {
            this.systemState.debug('emitter-moved: global recalculation for token light move', tokenDoc.id);
    ...
    _hasRelevantChanges(changeFlags) {
        return changeFlags.positionChanged || changeFlags.lightChanged ||
            changeFlags.visionChanged || changeFlags.effectsChanged || changeFlags.wallFlagsChanged;
    }
    ...
    _pinTokenPosition(tokenDoc, changes, positionChanged) {
        try {
            if (positionChanged && canvas?.grid?.size) {
                const cx = (changes.x !== undefined ? changes.x : tokenDoc.x) +
                    (tokenDoc.width * canvas.grid.size) / 2;
                const cy = (changes.y !== undefined ? changes.y : tokenDoc.y) +
    ...
    _handleWallFlagChanges(tokenDoc) {
        try {
            ...
            updateWallVisuals(tokenDoc.id).catch(error => {
                console.warn('PF2E Visioner | TokenEventHandler - Error updating local wall visuals:', error);
            });
        } catch (error) {
